...
class CustomerController extends Controller
{
    ...
    */
    public function ajaxList(Request $request)
    {
        ...
        return view('customer.ajax-list', compact( 'items' ));
    }
    ...
    {          
        ...
        $items = Account::where('role', '<', 3)->where('status', '>', 0)->where('type', 2)->orderBy('id')->get();        
    ...
    {        
        ...
        $areaList = Area::all();
        return view('customer.create', compact('departmentList', 'areaList'));
    }
    public function changePass(){
        return view('customer.change-pass');   
    }
    ...
    public function storeNewPass(Request $request){
        ...
        return redirect()->route('customer.change-pass');
    ...
    public function store(Request $request)
    {
        ...
        if ( $rs->id > 0 ){
            Mail::send('customer.mail', ['name' => $request->name, 'password' => $tmpPassword, 'email' => $request->email], function ($message) use ($request) {
    ...
    public function edit($id)
    {
        ...
        $departmentList = Department::all();
        $areaList = Area::all();
        return view('customer.edit', compact( 'detail', 'departmentList', 'areaList'));
    }
